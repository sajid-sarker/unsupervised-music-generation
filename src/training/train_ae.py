...
from src.config import AEConfig, DataConfig, PathConfig, ensure_output_dirs, get_device
...
def train(args: argparse.Namespace) -> None:
	...
	cfg = AEConfig(epochs=args.epochs, batch_size=args.batch_size, lr=args.lr)
	...
	for epoch in range(cfg.epochs):
		model.train()
		running = 0.0
		for (x,) in tqdm(train_loader, desc=f"AE Epoch {epoch+1}/{cfg.epochs}"):
			x = x.to(device)
			recon, _ = model(x)
	...
	torch.save(
		...
		paths.checkpoints_dir / "ae.pt",
	)

	save_loss_curve(train_losses, paths.plots_dir / "ae_train_loss.png", "AE Reconstruction Loss", "BCE Loss")
	save_loss_curve(val_losses, paths.plots_dir / "ae_val_loss.png", "AE Validation Loss", "BCE Loss")
	print(f"Saved checkpoint to {paths.checkpoints_dir / 'ae.pt'}")


def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Train LSTM Autoencoder for music generation.")
